...
def plate_detection(img, i):
    ...
    for box, conf in zip(boxes, confs):
        ...
        x,y,w,h = object   
        cv2.imwrite(vid + "_" + run +"_vehicle_" + i + 'detected_plate_' + str(count) + "_conf_" + conf + '.jpg', img[y:y+h, x:x+w])
        count += 1
    ...
    while success:
        ...
        if n % 20 == 0:
            ...
            for tup in zip(res_boxes,res_masks,res_conf):
                ...
                object = [int(box[0]), int(box[1]), int(box[2]- box[0]), int(box[3] - box[1])]
                x,y,w,h = object
                cv2.imwrite(vid + "_" + run +"_vehicle_" + str(i) + "_with_conf_" + str(conf)+".jpg", image[y:y+h, x:x+w])
                plate_detection(image[y:y+h, x:x+w], str(i), str(conf))
                        # break
        if n > 150:
            break
    end = time.time()
...
if __name__ == "__main__":
    ...
    if name == "utube.mp4":
        results = model(image, save=True)  # list of Results objects
        boxes = results[0].boxes.xyxy.tolist()
